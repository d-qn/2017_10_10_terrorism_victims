...
deathsByMonth <- dm %>% group_by(yearMonth) %>% 
  ...
  mutate(
    tot = cumsum(monthtot),
    label = paste0("Morts: ", monthtot, " (depuis 2012: ", tot, ")")
  )
...
plotSingleTime <- function(dm, labels, ym, txt) {
  ...
  p <- ggplot() + 
    swi_dark_map(base_size = 20) + 
    geom_map(
      data=world, map=world,
      aes(x=long, y=lat, map_id=region),
      color="#333333", fill= "#262626", size=0.4, alpha=0.9) +
    coord_map(xlim = xlim, ylim = ylim, projection = "gilbert") + 
    geom_jitter(data = bg, aes(x = lon, y= lat, size = z, alpha = as.numeric(date)), 
                stroke = 0, colour = "#6a332f") +
    geom_jitter(data = dd, aes(x = lon, y= lat, size = z), 
                alpha=0.5, stroke = 0, colour = "#d09995") +
    annotate(
      geom = "text", x = 0, y = 40, label = ym, colour = "lightgrey",
      alpha = 0.1, size = 50, hjust = 0.5, vjust = 0.5
    ) +
    # geom_map(
    #   data=world, map=world,
    #   aes(x=long, y=lat, map_id=region, group = 1),
    #   color="#4d4d4d", fill= "#24a8a8", size = 0.2, alpha=0.1
    # ) + 
    scale_size_continuous(
      trans = "log1p", range = c(10^-6, 7), 
      breaks = c(10, 100, 1000), limits = range(dm$z), 
      guide = guide_legend(override.aes = list(fill = "#d09995"))) +
    scale_alpha_continuous(range = c(0.05, 0.35), guide = F) +
    theme(
      legend.key.size = unit(1, "lines"),
      legend.position = c(0.85,0.12),
      legend.box.background = element_rect(size = 0, fill = "black"),
      plot.title = element_text(margin = margin(t = 7, b = -10, l = -10)),
      plot.subtitle = element_text(margin = margin(t = 20, b = -60, l = -10)),
      plot.margin = unit(c(0.5, 0.2, 0, 0), "cm")
    ) + 
    geom_text_repel(
      data = labels, aes(x = lon, y = lat, label = country_txt), 
      colour = "white", segment.alpha = 0.3,
      alpha = 0.5, nudge_x = -8, nudge_y = -6) +
    labs(title = "Morts du terrorisme 2012-2016", 
         subtitle = stats.label$label)
   print(p)
}
plotSingleTime(dm, labels, ym)

saveGIF({
  for(ym in unique(dm$yearMonth)) {
    plotSingleTime(dm, labels, ym)
  }
},  movie.name = paste0("terrorismDeathMap_", 'FR', ".gif"), interval = 0.8, 
  ani.width = 640*1.9, ani.height = 640, loop = TRUE)
