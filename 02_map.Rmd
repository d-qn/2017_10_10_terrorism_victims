...
translation.file <- "input/terrorism world map - Sheet1.csv"
...
plotSingleTime <- function(dm, labels, euro, ym, txt, lang) {
  ...
  p <- ggplot() + 
    swi_dark_map(base_size = 18, base_family = txt['base.font', lang], 
                 title_family = txt['title.font', lang], subtitle_family = txt['base.font', lang]) + 
    geom_map(
      data=world, map=world,
      aes(x=long, y=lat, map_id=region),
      color="#404040", fill= "#262626", size=0.35, alpha=0.85) +
    ...
    geom_jitter(data = dd, aes(x = lon, y= lat, size = z), 
                alpha=0.55, stroke = 0, colour = "#d09995") +
    scale_size_continuous(
      trans = "log1p", range = c(10^-8, 8), 
      breaks = c(10, 100, 1000), limits = range(dm$z), 
      guide = guide_legend(override.aes = list(fill = "#d09995"), title = txt['descr1', lang])) +
    scale_alpha_continuous(range = c(0.001, 0.3), guide = F) +
    theme(
      legend.key.size = unit(1, "lines"),
      legend.position = c(0.03,0.1),
      legend.key.height = unit(1.2, "lines"),
      plot.title = element_text(hjust = 0.5, colour = "#f2f2f2"),
    #  plot.subtitle = element_text(hjust = 0.5, margin = margin(b = -20), size = 30),
      plot.margin = unit(c(0.5, 0.2, 0.2, 0), "cm")
    ) + 
    geom_text_repel(
      data = labels, aes(x = lon, y = lat, label = label), 
      colour = "white", segment.alpha = 0.3, force = 2,
      alpha = 0.9, nudge_x = -9, nudge_y = -5, family = txt['base.font', lang], size = 5) +
    labs(title = txt['title', lang], caption = caption) +
    annotate(geom = "text", x = mean(xlim)-40, y = round(ylim[2])-4, label = yearMonth, 
             family = txt['base.font', lang], size = 12, colour = "white", hjust = 1, vjust = 0) +
    annotate(geom = "text", x = mean(xlim), y = round(ylim[2])-4, label = subtitle, 
             family = txt['title.font', lang], size = 11, colour = "#EBDDDD", hjust = 0.5, vjust = 0) +
    annotate(geom = "text", x = 80, y = round(ylim[2])-4, label = subtitle2, 
             family = txt['base.font', lang], size = 7, colour = "white", hjust = 1, vjust = 0, alpha = 0.5)
   #print(p)
  if(nrow(euro.label) > 0) {
    p2 <- p + 
      geom_text_repel(
       data = euro.label, aes(x = lon, y = lat, label = label), 
        colour = "white", segment.alpha = 0.3, force = 2,
        alpha = 0.9, nudge_x = 5, nudge_y = -4, family = txt['base.font', lang], size = 5)
    print(p2)
  } else {
    print(p)
  }
...
for(lang in colnames(txt)) {
  ...
  },  movie.name = paste0("terrorismDeathMap_", lang, ifelse(dev == T, "_dev", ""), ".gif"), 
  interval = 1.2, ani.width = 660 * 2, ani.height = 660 * 1.15, loop = TRUE)  
}

```

```{r find events only in Europe}
if(dev) {
