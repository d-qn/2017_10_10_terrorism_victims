...
plotSingleTime <- function(dm, labels, euro, ym, txt, lang) {
  ...
  sizeLabel <- if(lang %in% c('ZH', 'JA')) 5.5 else 6.3
  ...
  p <- ggplot() + 
    swi_dark_map(base_size = 20, base_family = txt['base.font', lang], 
    ...
    scale_alpha_continuous(range = c(0.001, 0.25), guide = F) +
    theme(
      legend.key.size = unit(1, "lines"),
      legend.position = c(0.05,0.07),
      legend.key.height = unit(1.2, "lines"),
      plot.title = element_text(hjust = 0.5, colour = "#f2f2f2",  margin=margin(b=topMargin, t = 10), 
                                size =  38),
      plot.subtitle = element_text(hjust = 0.5, size = 43),
      ...
      plot.caption = element_text(size = 18, hjust=1, margin=margin(t=13, b=10, r=25), color = '#f2f2f2')
    ) + 
    geom_text_repel(
      data = labels, aes(x = lon, y = lat, label = label), 
      colour = "white", segment.alpha = 0.4, force = 2,
  ...
  p <- p + annotate(geom = "text", x = 0, y = 85, label = subtitle2,
                    family = txt['base.font', lang], size = 9, 
    ...
    p2 <- p + 
      geom_text_repel(
       data = euro.label, aes(x = lon, y = lat, label = label), 
        colour = "white", segment.alpha = 0.5, force = 2,
...
for(lang in colnames(txt)) {
  ...
  interval = 1, ani.width = 660 * 2, ani.height = 660 * 1.5, loop = TRUE)
